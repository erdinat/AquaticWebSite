...
import { DownOutlined, UpOutlined, AppstoreOutlined, SettingOutlined, CameraOutlined, BulbOutlined } from '@ant-design/icons';
...
const getCategoryIcon = (id) => {
    switch (id) {
        case 'subsea-connectors': return <SettingOutlined />;
        case 'underwater-cameras': return <CameraOutlined />;
        case 'subsea-lights-lasers': return <BulbOutlined />;
                ...
                <div className="container">
                    <Layout className="products-layout" style={{ background: 'transparent' }}>
                        <Sider
                            ...
                            theme="light"
                        >
                            <h3 className="sider-title">{t('products.filterTitle', 'Categories')}</h3>
                            <Menu
                                mode="inline"
                        ...
                        <Content className="products-content">
                            {filteredProducts.length === 0 ? (
                                <Empty
                                    description={t('products.noProducts', 'No products found in this category.')}
                                    className="reveal"
                                />
                            ) : (
                                <>
                                <Row gutter={[28, 28]}>
                                    {pagedProducts.map((product) => {
                                        ...
                                        const accentColor = categoryColors[product.categoryId] || 'var(--color-primary)';
                                                    ...
                                                    <Card
                                                        ...
                                                        cover={
                                                            <div className="product-image-wrapper">
                                                                ...
                                                                />
                                                                {product.isPremium && (
                                                                    <div className="product-premium-badge">Premium</div>
                                                                )}
                                                            </div>
                                                        }
                                                    >
                                                        ...
                                                        >
                                                            {isExpanded
                                                                ? t('products.viewLess', 'View Less')
                                                                : t('products.viewMore', 'View Details')}
                                                                ...
                                                                <table className="harness-specs-table">
                                                                    <tbody>
                                                                        {Object.entries(product.specs || {}).map(([key, value]) => (
                                                                            <tr key={key}>
                                                                                <td className="harness-spec-label">{key}</td>
